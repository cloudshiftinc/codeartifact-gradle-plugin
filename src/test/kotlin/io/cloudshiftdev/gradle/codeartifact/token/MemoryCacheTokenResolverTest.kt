...
class MemoryCacheTokenResolverTest :
    FunSpec({
        test("caching works") {
            ...
            val cache = MemoryCacheTokenResolver(delegate)
            val resolvedToken = cache.resolve(token.endpoint)
            resolvedToken shouldBe token
